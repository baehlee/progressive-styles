...
| **3** | Homepage "Trusted Experts" + "Work With Us" rewrites | 🟡 Deliverables ready (`pages/homepage/trusted-experts.html` + `work-with-us.html`). Trusted Experts = branded "why us" band (italic-terracotta heading, 80+/4/MN&WI/No-Hate stat strip, 4 general tiles Buying/Selling/Start-to-finish/Built-for-everyone). Work With Us = full-bleed navy closing CTA (4 founder portraits, hero-echoing "place to belong," CTAs → `/evaluation` + `/team`). Scoped #ids, no inline SVG, verified at 390px + desktop. Pending: human pastes both into Lofty + hides the two stock sections they replace. | Custom HTML blocks |
| **4** | Team page rebuild with real bios + video integration | 🟡 Deliverable ready (`pages/team/team.html`): four founders as alternating photo/bio rows, each headshot a click-to-play welcome video (same embeds + lightbox as the hero), ≤150-word web bios (in `content/founders/web-versions.md`), roles read "Co-Founder". Scoped `#preg-team`, mobile-first (verified 1440 + 390px), lightbox verified, no inline SVG, hardened against the hosted.css global button/heading rules. Pending: human pastes under the stock `md-agent-search` "OUR AGENTS" module on /team (and optionally hides it). | Custom HTML blocks + content |
| **5** | No Hate Pledge dedicated landing page | 🟡 Deliverable ready (`pages/mission/no-hate-pledge.html`, scoped `#preg-pledge`): navy hero ("No hate. No exceptions."), the formal mission statement signed by the four founders, a "who we serve" grid (LGBTQ+, Spanish-speaking/multilingual, first-gen, minority/immigrant, accessibility-conscious), a fair-housing note, and a navy CTA band linking /listing + /sell + /team. SEO keyword "inclusive real estate agent minneapolis" woven in. Mobile-first (verified 1440 + 390px), no inline SVG, dark-band text + CTA hardened against the hosted.css global rules. Pending: human creates a /no-hate-pledge custom landing page and pastes it. | Custom landing page |
| **6** | Sell page rewrite | 🟡 Deliverable ready (`pages/sell/sell.html`, scoped `#preg-sell`): value-prop intro ("Know what it's worth. Get every dollar.") with a primary CTA to /evaluation, the six-step selling process with real copy, and a navy "why sell with us" band (construction lens, financial lens, marketing, negotiation) + CTAs to /evaluation + /team. Self-contained (no hosted.css change), mobile-first (verified 1440 + 390px), no inline SVG, dark-band + CTA hardened. Pending: human pastes it and hides the stock `md-sell-banner` + `md-sell-process` modules. | Custom HTML blocks + content |
| **7** | First identity landing page (LGBTQ+ relocation as template) | 🟡 Deliverable ready (`pages/identity-landing/lgbtq-realtor-twin-cities.html`, scoped `#preg-lgbtq`): ~1,210-word landing page for /lgbtq-realtor-twin-cities. Navy hero, intro (keywords: lgbtq/gay realtor twin cities, lgbt-friendly agent MN), Travis welcome-video feature (lightbox), "what affirming means" list, relocating-to-the-Twin-Cities section (real MN facts), how-we-help with internal links (/listing /sell /evaluation /no-hate-pledge /team), a 6-question FAQ, navy CTA band. This is the TEMPLATE for the remaining identity pages — duplicate + swap founder/video/copy. Mobile-first (verified 1440 + 390px), lightbox verified, no inline SVG, dark-band + CTA hardened. Pending: human creates the landing page + pastes. | Custom landing page |
| **8** | SEO foundation: Areas pages + identity pages buildout + blog cadence | 🟢 In progress — most built. ✅ All 10 tier-one **Areas** pages (`pages/areas/`, `#preg-area`, 554–666 words original each, complement Lofty's auto market widget + listings). ✅ All 5 **identity** pages (`pages/identity-landing/`: lgbtq, spanish, first-time, relocating, mortgage — 910–1,213 words each, founder video features). ✅ **SEO META master sheet** for every page (`deliverables-pending/SEO-META-SHEET-all-pages.md`) + site-wide SEO Setup checklist. ⏳ Remaining: **blog cadence** (2–4 posts/mo, 3 rotating types — see `content/seo/strategy.md §6`). | Bulk content generation |
